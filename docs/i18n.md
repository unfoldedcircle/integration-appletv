...
in [i18n.py](../src/i18n.py) which complement the common `_` translation function:
...
```shell
xgettext -d src -o src/locales/src.pot --from-code=UTF-8 --language=Python \
...
```shell
LANGUAGE=en_US
msgmerge --no-wrap -U src/locales/$LANGUAGE/LC_MESSAGES/src.po \
    src/locales/src.pot
```
...
```shell
LANGUAGE=en_US
msgfmt src/locales/$LANGUAGE/LC_MESSAGES/src.po \
    -o src/locales/$LANGUAGE/LC_MESSAGES/src.mo
```
...
msginit --no-wrap -i src/locales/src.pot \
    -o src/locales/$LANGUAGE/LC_MESSAGES/src.po \
    -l $LANGUAGE
```
