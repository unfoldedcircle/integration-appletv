...
This project uses the Python [gettext](https://docs.python.org/3.11/library/gettext.html) module for
internationalization.

- SimpleLocalize is used to translate texts: https://simplelocalize.io/suggestions/?id=19205eea884c4c9d9e0e85957382d0a0
- English is used as the reference language for all translations.
    ...
    - Exceptions are for very long texts, where a summary text is enough, or omitting web-links or other formatting
      options.
...
- `--no-wrap` is used for better SimpleLocalize merge compatibility.
...
‼️ Only the English reference language `en_US` should be updated. All other language files are handled by
SimpleLocalize!
...
‼️ This is for reference and local use only. New project translation languages must be requested and created in
SimpleLocalize.
